package com.b2c.prototype.processor.item;

import com.b2c.prototype.modal.dto.payload.constant.ArticularStatusDto;

import java.util.List;
import java.util.Map;

public interface IArticularStatusProcess {
    void persistEntity(final Map<String, Object> payload);
    void mergeEntity(final Map<String, Object> payload, final String value);
    void removeEntity(final String value);

    List<ArticularStatusDto> getEntityList(final String location);
    ArticularStatusDto getEntity(final String location, final String value);
}
